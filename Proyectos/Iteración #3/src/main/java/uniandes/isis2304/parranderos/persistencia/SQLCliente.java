...
class SQLCliente {
	...
	// RFC8
	public List<Object> encontrarClientesFrecuentesPorIdAlojamiento(PersistenceManager pm, long idAlojamiento)
    {
        Query q = pm.newQuery(SQL, "SELECT A_RESERVA.IDENTIFICACIONCLIENTE FROM A_RESERVA WHERE (A_RESERVA.IDENTIFICACIONCLIENTE = (SELECT A_RESERVA.IDENTIFICACIONCLIENTE FROM A_RESERVA WHERE (A_RESERVA.IDALOJAMIENTO = ?) GROUP BY A_RESERVA.IDENTIFICACIONCLIENTE HAVING COUNT(A_RESERVA.IDENTIFICACIONCLIENTE) >= 3) OR A_RESERVA.IDENTIFICACIONCLIENTE = (SELECT A_RESERVA.IDENTIFICACIONCLIENTE FROM A_RESERVA WHERE A_RESERVA.IDALOJAMIENTO = ? GROUP BY A_RESERVA.IDENTIFICACIONCLIENTE HAVING 15 <= SUM(A_RESERVA.FECHAFIN - A_RESERVA.FECHAINI))) AND A_RESERVA.IDALOJAMIENTO = ? GROUP BY A_RESERVA.IDENTIFICACIONCLIENTE");
        q.setParameters(idAlojamiento, idAlojamiento, idAlojamiento);
        return q.executeList();
    }

	/**
	 * RFC6 - Uso para un usuario dado
	 */

	public List<Object[]> usoParaUnUsuario(PersistenceManager pm, String identificacion)
	{
		Query q = pm.newQuery(SQL, "SELECT A_CLIENTE.IDENTIFICACION, COALESCE(A_RESERVA.FECHAFIN - A_RESERVA.FECHAINI,0) DIAS_R, A_RESERVA.IDALOJAMIENTO, A_RESERVA.GANANCIA DINERO_PAGADO, A_ALOJAMIENTO.TIPOALOJAMIENTO, LISTAGG(A_SERVICIO.TIPO, ',') SERVICIOS "
									...
									+ " WHERE A_CLIENTE.IDENTIFICACION = ? "
